...
export default function ProductCard({ sortOption }) {
  ...
  const [currentImageIndex, setCurrentImageIndex] = useState({}); // Track current image for slideshow
  const [isHovering, setIsHovering] = useState({}); // Track hover state per product
  ...
  const { fetchMobileNumbers, mobileNumbers } = useCategoryStore();
  ...
  // Get current image based on hover or selected color
  const getCurrentImage = useCallback(
    (product) => {
      const variant = product.variants?.[0];
      if (!variant?.images?.length) {
        console.warn(`No images available for product ${product._id}. Using fallback image.`);
        return { url: fallbackImage, isVideo: false };
      }

      const imageIndex = isHovering[product._id]
        ? currentImageIndex[product._id] ?? 0
        : (selectedColorIndex[product._id] ?? 0) < variant.images.length
      ...
      if (isHovering[product._id] && imageIndex >= variant.images.length) {
        console.warn(
          `Invalid image index ${imageIndex} for product ${product._id}. Falling back to primary or first image.`
        );
      }
    ...
    [selectedColorIndex, currentImageIndex, isHovering, baseURL, fallbackImage]
  );
  ...
  const getFinalPrice = useCallback(
    (product) => {
      const variant = product.variants?.[0];
      return variant?.finalPrice ?? product.base_price ?? 0;
    },
    []
  );
  ...
  //   Object.keys(isHovering).forEach((productId) => {
    ...
    if (products && products.length > 0) {
      ...
      products.forEach((product) => {
        if (product.variants?.[0]?.color?.length > 0) {
          indexMap[product._id] = 0; // Default to first color
        }
      });
      setSelectedColorIndex(indexMap);
    }
  }, [products]);
  ...
  const handleColorClick = useCallback((productId, colorIndex) => {
  ...
  const handleCardHover = useCallback((productId) => {
    ...
    setCurrentImageIndex((prev) => ({
      ...prev,
      [productId]: 0, // Start slideshow from first image
    }));
  }, []);

  const handleCardHoverEnd = useCallback((productId) => {
    setIsHovering((prev) => {
      const newState = { ...prev };
      delete newState[productId]; // Stop slideshow
      return newState;
    });
    setCurrentImageIndex((prev) => {
      const newState = { ...prev };
      delete newState[productId]; // Reset to selected color image
      return newState;
    });
  }, []);
    ...
    <div className="max-w-7xl mx-auto p-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {sortedProducts.map((product) => {
          ...
          const selectedColor = product.variants?.[0]?.color?.[selectedColorIndex[product._id]]?.name;
          const { url: mediaUrl, isVideo } = getCurrentImage(product);

          return (
            <Link href={`/fashion/${product._id}`} key={product._id}>
              <div
                className=" rounded hover:shadow-lg transition cursor-pointer p-4"
                onMouseEnter={() => handleCardHover(product._id)}
                onMouseLeave={() => handleCardHoverEnd(product._id)}
              >
...
🧮 *Quantity:* 1`;
                        ...
                        const phoneNumber = mobileNumbers?.[0]?.mobileNumber || process.env.NEXT_PUBLIC_PHONE_NUMBER;
                ...
                <div className="flex gap-2 flex-wrap">
                  {product.variants?.[0]?.color?.map((color, index) => (
                    <button
...
const StableFilterHeader = ({ sortBy, onSortChange, onTagChange, activeTag }) => {
  ...
  const { activeFilters, setSearchQuery, fetchProductsByFilters } = useFilter();
  ...
  // Sync searchValue with activeFilters.search on mount
  useEffect(() => {
    ...
    (query) => {
      ...
      fetchProductsByFilters({
        ...
        .catch((err) => {
          ...
          if (err.name !== 'AbortError') {
            toast.error(
              query && err.message.includes('No products')
