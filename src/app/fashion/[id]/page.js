...
export default function ProductPage() {
  ...
  useEffect(() => {
    if (product?.variants?.length > 0) {
      ...
      const newColor = firstVariant.color?.[0]?.name || "";
      const newSize = firstVariant.sizes?.[0]?.size || "";
      setSelectedColor((prev) => {
        // console.log("Setting selectedColor:", newColor);
  ...
  useEffect(() => {
    if (selectedColor && product?.variants?.length > 0) {
      const selectedVariant = product.variants[0]; // Single variant with multiple colors
      const newSize = selectedVariant?.sizes?.[0]?.size || "";
      setSelectedSize(newSize);
    }
  }, [selectedColor, product]);

  // Find the selected variant (single variant in this case)
  ...
  const primaryMediaIndex =
    mediaItems.findIndex((item) => item.isPrimary) !== -1
  ...
  // Construct colors array from variant.color array
  const colors = product.variants?.flatMap((variant) =>
    variant.color?.map((color) => ({
      name: color.name || "Unknown",
      hex: color.hex || "#000000",
      id: color._id || variant._id,
    })) || []
  ) || [];
  ...
  return (
    <>
      <div className="max-w-[1362px] mx-auto px-5 bg-white pt-35">
        <div className="mt-2  ">
          <Breadcrumb>
            <BreadcrumbList>
              <BreadcrumbItem>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-9 pt-5">
          {/* LEFT MEDIA (IMAGES & VIDEOS) */}
          <div className="flex flex-col lg:flex-row gap-4">
            <div
              className="mt-2 hidden lg:flex flex-col space-y-2 overflow-y-scroll max-h-[544px] w-19"
              style={{
                scrollbarWidth: "none",
            ...
            >
              {mediaItems.map((item, index) => (
                <button
                  ...
                  className={`w-16 h-20 border-2 cursor-pointer flex-shrink-0 relative ${
                    currentMedia === index ? "border-blue-500" : "border-gray-200"
              ...
              <div className="lg:hidden flex space-x-2 overflow-x-auto mt-3">
                {mediaItems.map((item, index) => (
                  <button
                    key={`${item._id}-${index}`}
                    onClick={() => setCurrentMedia(index)}
                    className={`flex-shrink-0 w-16 h-20 border-2 relative ${
                      currentMedia === index ? "border-blue-500" : "border-gray-300"
